...
public partial class MannequinScene : Node3D
{
	...
	//-------------------------------------------------------------------------
	// OnChangModel: selects which model will show based on selection
	//			     from dropdown
	//-------------------------------------------------------------------------

	public void OnChangeModel(long index)
	{
		GD.Print("OnChangeModel");
		if(model!=null)
		{
			model.Hide();
		}
		...
		switch(mcType){
			// when new ManneControl classes created, add them to the list
			case ManneControlType.SimpleBC:
				mcObject = new MCTestSimpleBC(modelItf,this);
				break;
			case ManneControlType.JointControl:
				mcObject = new JointControl(modelItf,this);
				break;
